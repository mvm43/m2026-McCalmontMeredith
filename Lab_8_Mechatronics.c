...
// Functions
void step_CW( float angle , uint32_t time_ms );
void step_CCW( float angle , uint32_t time_ms );
...
{
   // Setup
	DDRD = 0b11110000; // Sets pins 4, 5, 6, and 7 output, and 	pins 0, 1, 2 and 3 to input. Pins 4-7 will be used for 	stepper motor control (CORRECT)
	DDRB = 0b11111100; //Good
	DDRC = 0b11111110;
   while(1)
   {
	    while (!(PINC & (1 << PC0)));
...
void step_CCW( float angle , uint32_t time_ms )
{
	...
	// phase2a = PORTD_5, phase2b = PORTD_4
	    int steps = angle / 1.8;
	    uint32_t delay_per_step = time_ms / steps;

	    PORTB = 0b00000001; // LED ON (CW)
